 ...
 *
 * 流程：
 *   1. 调用 npm run convert:built-in，完成转换并合并到仓库内 TouHouLittleMaid_RP
 *   2. 用新的 TouHouLittleMaid_RP 覆盖
...
async function main() {
  ...
  console.log('步骤 1/2: 执行内置包转换（npm run convert:built-in）');
  await runConvertBuiltIn();

  console.log('步骤 2/2: 覆盖 Minecraft 开发资源包');
