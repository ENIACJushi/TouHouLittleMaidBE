...
- [ ] 都完成后，再加个坐垫包转换，应该比较容易
  ...
  - [ ] 实体不要随玩家转动
  - [ ] 离开屏幕区域没有渲染，参考祭坛修改。
  - [ ] 收回逻辑：潜行攻击收回，有模型包记忆功能，放下的坐垫被回收后会记录使用的模型，重新放置会保持模型
- [ ] 调整女仆模型包的lang key 格式，需要同步修改手动修改的东方包

- 使用物品 "touhou_little_maid:chair" 放置，放置由脚本处理，规则为：
  - 潜行放置时，实体精准生成在玩家点击的位置，并面向玩家
  - 非潜行放置时：
    - 实体只会朝向八个方向：x+、x-、z+、z-，以及它们之间的45度角，根据玩家朝向确定
    - 若点击的是方块上表面，则实体生成在玩家点击的方块的上表面中央
    - 若点击的是方块侧面，则取该侧面紧贴的方块的底面中央
    - 若点击的是方块下表面，则实体生成在点击方块之下的方块底面中央

放置下活板门，点击顶部得到坐标(0.47, 0.82, 0.45)
放置上活板门，点击底部得到坐标(0.36, 0.18, 0.46)，确定y轴原点在方块顶部

放置靠西的活板门，点击东面得到坐标(0.82,0.37,0.54)
放置靠东的活板门，点击西面得到坐标(0.18,0.58,0.65)，确定x轴原点在方块东面

放置靠北的活板门，点击南面得到坐标(0.47,0.5,0.18)
放置靠南的活板门，点击北面得到坐标(0.45,0.53,0.82)，确定z轴原点在方块北面
...
2、创造模式下使用物品「记忆中的幻想乡（管理）」打开管理面板（也可 `/scriptevent thlm:manage`），进入「皮肤包」，粘贴网站给出的 JSON 并提交。
