...
- 灵梦油库里史莱姆
- 女仆处于屏幕外也会有动画，为更多能在UI内正常播放的动画提供条件
- 处于战斗模式的女仆会被所有能主动攻击玩家的怪物锁定
- 平衡调整：降低近战攻击频率
- 取消激活祭坛时背景必须为空气的要求
- 优化：用数驱代替脚本进行P点吸收扫描，减小背景运算压力
- 四种环境音效：冷、热、雨、雪
- **修复使用魂符/相机收回女仆时，背包内的物品不会掉落的 Bug**
  - 这个bug还会导致收回女仆时残留透明背包实体，使用如下命令可以将此类背包聚集到最近的一个玩家（将 @p 换为玩家名即可聚集到指定玩家）
  -   /execute as @e[type=touhou_little_maid:maid_backpack] at @s unless entity @e[r=2,type=thlmm:maid] run tp @p 
  -  正常掉落的背包也会被聚集，需要手动筛选。
...
- 繁殖动物模式
  在实体的born事件中对其父母进行标记
  对于玩家喂养，使用love_filters过滤被女仆喂养过的动物
  对于女仆喂养，需要先找到非攻击式追随行为，利用用标记过滤

- 庭灯：可以存储P点，打掉之后P点会以lore的形式存储在物品之中（需要测试lore中是否能包含translate文本）
