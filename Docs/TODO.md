...
✓ 优化寻主方式
 - 背包只能由女仆的主人打开；
 - 根据设为驯服时，最近玩家为主人的原理，取消魂符、相片放置时再驯服一次的要求。（跟随式寻主依旧是需要的，因为第一次驯服时脚本并不知道主人是谁，需要靠女仆自己找）

✓ 降低女仆回血频率 6秒/次 → 9秒/次

√ 弹幕的亮度不会因处于暗处而变低

√ 完善女仆数据存储，升级版本时即使不立即升级模组也能保留数据

√ 攻击目标白名单：女仆、玩家、村民、悦灵、马、驴、骡、美西螈、猫、豹猫、鹦鹉、骆驼、熊猫、其他被驯服的生物（不会主动攻击，但被攻击时仍会反击）

√ 反击目标白名单：女仆、玩家、其他被驯服的生物 (即使自身或主人被这些生物攻击，也不会反击)
