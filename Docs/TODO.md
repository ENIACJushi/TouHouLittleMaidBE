...
下一步：
1. 添加背包物品、换包交互并完善背包的渲染。
2. 完善女仆个人背包与背包实体之间物品的传递，同时修改阵亡/收回时的相关处理
...
home、拾物模式是独立于工作模式的模式，如何在ui读取还有待考量，消耗掉一个mark_varian有些浪费，可能会用tag标注
home模式用minecraft:is_baby标记，为true时是home模式。
...
在npc ui里加上一个判断，让普通玩家能进入模型选择页，靠npc来设置皮肤


背包另做一个实体，让它骑在女仆上，免疫一切伤害
有隐形包、小背包、中背包、大背包四种，当主人潜行时，背包的碰撞箱变大，使得玩家与女仆交互时会点到包
容量分别为6、12、24、36
隐藏背包功能，放在女仆菜单里
...
true backpack把背包集中存储到了世界的一个位置，用一个自带加载区域的实体来保证背包能被一直加载
或许有更好的办法：在转化为物品时女仆所处的位置生成一个临时实体来存储背包
当调用物品召唤女仆，把一个自带常加载的实体传送过去，实现按需加载。
...
拟定的结构：touhou_little_maid:maid_define_xxx使用variant定义所有的女仆实体，然后在脚本里设置特定variant对应的特殊女仆命名空间，在variant变更时会检测是否为特殊定义，若是则发生转变。
    特殊定义的格式为 touhou_little_maid:maid_special_xxx
