...
本次更新对弹幕进行了加强，包含伤害累积机制与全新的御币发射模式！

伤害累积机制：当弹幕击中因受击而无敌的生物时，会累积伤害，当无敌时间结束后，无敌期间累积的伤害会一次释放。
这一机制将会在高射速和大量弹幕同时发射的场景发挥作用，有效地避免漏伤害的情况。

全新御币发射模式：只需按着不动——符札会自动地连续发射，并且速度比以前更快。而更快的不止有弹速！御币对移速降低作用较小，允许在发射过程中进行更好的机动。
你还可以为它附上弩类附魔：多重射击会同时发射三排弹幕；穿透可以让符札在击中生物后穿透它，对后方更多的生物造成伤害；快速装填则将发射频率提高到更高的水平。
力量和虚弱效果也能对弹幕生效，倍率和弓弩一致。
新御币的定位是中近距离交战，对于远处的高机动目标，可能并不能像传统的远程武器（如弓）一样地准确命中它们。控制好交战距离可以更好地发挥御币的优势~

除了普通御币，自动连发、附魔效果和药水效果现在也在樱之御币生效了，可以用它来精准杀伤较远的目标
