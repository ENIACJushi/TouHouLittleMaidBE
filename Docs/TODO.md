...
适配1.20.50
取消弹幕、女仆、P点在世界启动时注册动态属性的步骤。
为女仆生成时间增加设置默认属性的步骤。
移动 itemDefinitionEvent 事件下的业务：
hgt：御币弹种切换，本来就没有走事件
hga：合成台御币激活，移动到使用事件
sc：符卡施放，移动到使用事件，改用前缀“thlms:”识别
is：物品弹幕，改为单独识别
八卦炉由弹幕物品改为符卡。物品id修改为“thlms:gossip”。
将女仆转化为物品时，保存工作模式，但由物品转化为女仆时，不会设置工作模式。

祭坛、首次使用魂符、生成蛋、指令召唤的女仆会随机选择一个皮肤。
魂符首次召唤的女仆现在也会只认召唤者为主人。
英文支持
