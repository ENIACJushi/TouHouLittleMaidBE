...
女仆本体：
 跟随模式：家、跟随玩家（用minecraft:is_baby标记）
 工作模式：空闲、近战
 自动回血：6秒回复一次，每次回复量 2~5
 背包显示开关
...
☆ 完善私有背包与背包实体之间物品的传递，同时修改阵亡/收回时的相关处理
☆ 基本工作模式：弹幕攻击、农场。 因为不好拿东西，弓兵模式或许会取消

☆ 弹幕编组：定类型：maid 无法伤害有主人的生物；monster 可以伤害所有生物。
☆ 属性点：护甲和武器的替代品。

- 完善新UI，皮肤变量改为 variant 和 mark_variant。哪个作为组号需要研究一下NPCUI
构建计分板配置系统
...
实体的装备栏不能获取，包括能穿甲僵尸，而玩家的就可以
考虑硬编码装备属性，将附魔读取、减伤、减耐久写进属性里，这样还能保证盔甲栏的唯一性，即 只有背包里是有盔甲物品的。
