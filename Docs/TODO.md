...
本次更新对弹幕进行了加强，包含伤害累积机制、全新的御币和女仆索敌范围增加等内容！

### 伤害累积机制

当弹幕击中因受击而无敌的生物时，会累积伤害，无敌时间结束后，无敌期间累积的伤害会一次释放。
这一机制将会在高射速和大量弹幕同时发射的场景发挥作用，有效地避免漏伤害的情况。

### 全新御币

- 瞬发：弹幕发射不再需要蓄力，当长按屏幕或右键时，会进入自动连发射击状态。
- 自动连发：只需按屏幕或右键不动，御币就会自动地连续发射，并且弹速比以前更快。
- 快速移动：御币对移速降低作用较小，允许在发射过程中进行更好的机动。
- 普通御币附魔：普通御币可以被附上弩类附魔，「多重射击」会同时发射三排符札；「穿透」可以让符札在击中生物后穿透它，对后方更多的生物造成伤害；「快速装填」则将发射频率提高到更高的水平。
- 樱之御币附魔：樱之御币可以被附上弓类附魔，「力量」可以提高伤害和穿透力；「火矢」将花瓣点燃，不同维度有不同颜色的火焰，受到攻击的生物会被点燃；「冲击」则将生物推动更远的距离。「无限」目前没有影响，因为御币射击本就不会消耗弹药。
- 药水效果：力量和虚弱效果也能对弹幕生效，倍率和弓弩一致。

普通御币的定位是中近距离交战，对于远处的高机动目标，并不能像传统的远程武器（如弓）一样地准确命中它们。控制好交战距离可以更好地发挥御币的优势~
樱之御币的定位是远距离精准射击，可以用它来精准杀伤较远的目标，其穿透属性则使其能压制密集的怪物。

### 其它更新内容

- 女仆索敌距离增加：过去，女仆的索敌范围只有 16，现在这个距离被增加到了48，可以锁定和攻击更远的目标。同时，尝试保持的距离也增加了，女仆会在更安全的距离射击。
- 女仆预判射击修正：在某次更新后，预判射击的提前量出现了错误，此次更新修正了这个错误
- 修复女仆坐下状态有时会乱跑的问题
...
NTR工具
右键女仆或墓碑，显示其信息，然后选择是否将其主人设为自己。（或输入的其他人）
当使用其他人的魂符或照片，将此工具放在副手可以强制召唤并将其主人设为自己。
...
逐步将其它交互事件放入cc中（御币的转换事件、祭坛的放置事件等）
收竹子模式

指南书前置库
指南书改用factory？
- 利用全局变量内一个记录模组uuid的量，并在factory设置ignore，区分不同模组提供的库，保证只有一个库在UI被计算
- 真按钮 旧状态记录可能可以删除，因为引入旧状态只是缩短了上升段，而上升段已经被最大值限制住了，在置0段才会赋值
- 书页过长时，可以滑动查看
√ 图片显示功能
√ 调整binding条件，once->visible always->always_when_visible
