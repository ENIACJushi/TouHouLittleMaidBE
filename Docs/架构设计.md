...
子弹、激光等类型的弹幕有着很大的相似度，但却并不完全相同，可以再增加一个 LineShapeBase，整合二者公用接口。

弹幕系统分为四层：
第一层 shape（弹幕类型） 提供 **具体类型弹幕** 的最基本接口，如子弹型弹幕包含实体创建、留存时间设置以及一些特殊弹幕的接口。目前类型基本可分为：直射、激光、范围（如地底太阳）。
第二层 shoot 提供面向单发射击的接口，负责维护发射者等弹幕之外的信息。
第三层 pattern（图案） 针对一类或几类有共同调用方法的 **抽象弹幕类型**，提供通用的发射方案。如对于直射型和激光型弹幕，都可以使用扇形图案来发射。
第四层 spellCards 提供较为复杂的、不易组合的成品符卡

发射一个弹幕时，首先要创建shapes实例，然后将它传递给shoots，通过shoots来发射弹幕
