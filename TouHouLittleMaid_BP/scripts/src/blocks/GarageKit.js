...
export class GarageKit {
    ...
    static activate(event) {
        ...
        for (; index--; index > 0) {
            ...
            if (this.match(dimension, size.space, startLocation, xSign, zSign)) {
                ...
                if (index === 0) {
                    // 实体
                    let maid = dimension.spawnEntity("thlmm:maid", new Vector(startLocation.x + 0.5, startLocation.y, startLocation.z + 0.5));
                    ...
                    maid.teleport(maid.location, { "facingLocation": new Vector(maid.location.x + facing[0], maid.location.y, maid.location.z + facing[1]) });
                    // 占位方块
                ...
                // 生成雕塑
                    ...
                    let endLocation = new Vector(startLocation.x + offset[0], startLocation.y + offset[1], startLocation.z + offset[2]);
                    // 实体
                    let maid = dimension.spawnEntity("thlmm:maid", new Vector(startLocation.x + offset[0] / 2 + 0.5, startLocation.y, startLocation.z + offset[2] / 2 + 0.5));
                    maid.setDynamicProperty("spawn_set", true);
                    // 设置大小
                    EntityMaid.Statues.scale.set(maid, size.scale);
                    EntityMaid.Statues.space.set(maid, new Vector(size.space[0], size.space[1], size.space[2]));
        ...
        for (let x = 0; x < space[0]; x++) {
            for (let y = 0; y < space[1]; y++) {
                for (let z = 0; z < space[2]; z++) {
                    try {
                        let block = dimension.getBlock(new Vector(startLocation.x + xSign * x, startLocation.y + y, startLocation.z + zSign * z));
                        if (!isClay(block)) {
                            return false;
                        }
                    }
        ...
        switch (EntityMaid.Work.get(maid)) {
            case -2:
                { // 雕塑 恢复构造前状态
                    ...
                    let startLocation = new Vector(maid.location.x - space.x / 2 + 0.5, maid.location.y, maid.location.z - space.z / 2 + 0.5);
                    let endLocation = new Vector(startLocation.x + space.x - 1, startLocation.y + space.y - 1, startLocation.z + space.z - 1);
                    // 使用fill，提高对大体积区域的检测速度
                    let statuesBlocks = maid.dimension.getBlocks(new BlockVolume(startLocation, endLocation), { "includeTypes": [blockStatues] }, true);
    ...
    static placeEvent(event) {
        ...
        // 放置方块
        dimension.fillBlocks(new BlockVolume(location, location), BlockPermutation.resolve(blockGarageKit, { "thlm:solid": true }));
        ///// 放置实体 /////
        location.x += 0.5;
        location.z += 0.5;
...
//# sourceMappingURL=GarageKit.js.map