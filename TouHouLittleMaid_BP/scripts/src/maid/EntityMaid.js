...
export class EntityMaid {
    ...
    static initDynamicProperties(maid) {
        ...
        if (DP.getVector(maid, "home") === undefined)
            DP.setVector(maid, "home", { x: 0, y: 0, z: 0 });
        if (DP.getInt(maid, "home_dim") === undefined)
            DP.setInt(maid, "home_dim", 0);
        if (DP.getInt(maid, "level") === undefined)
            DP.setInt(maid, "level", 1);
        if (DP.getInt(maid, "kill") === undefined)
            DP.setInt(maid, "kill", 0);
        if (DP.getBoolean(maid, "pick") === undefined)
            DP.setBoolean(maid, "pick", true);
    }
    /**
     * 初始化女仆实体为女仆
     * @param {Entity} maid
    ...
    static init_maid(maid, reborn = false) {
        if (maid.getDynamicProperty("spawn_set") !== undefined)
            return;
        // 添加女仆属性
    ...
    static toStr(maid, dump = true) {
        ...
        if (o_id !== undefined) {
            ...
            // 女仆名称
            if (this.getNameTag(maid) !== "")
                maidStr = StrMaid.Str.setMaidName(maidStr, this.getNameTag(maid));
        }
        // 爆出物品
        if (dump)
    ...
    static fromStr(maidStr, dimension, location, set_health = false) {
        ...
        if (maidName !== undefined) {
            maid.nameTag = maidName;
        }
        // 主人名称
        if (ownerName !== undefined) {
            this.Owner.setName(maid, ownerName);
        }
        ;
        // 设置主人
        if (hasOwner) {
...
EntityMaid.Backpack = (_a = class {
    ...
    __setFunctionName(_a, "Backpack"),
    _a.default = 0,
    _a.small = 1,
    _a.middle = 2,
    _a.big = 3,
    _a.capacityList = [6, 12, 24, 36],
    _a.nameList = [
        "default",
        "small",
        "middle",
...
//# sourceMappingURL=EntityMaid.js.map