...
export class MaidTarget {
    ...
    static targetAcquire(event) {
        ...
        switch (target.typeId.substring(6)) {
            case "sugar_cane":
                SugarCane.acquire(target, maid);
                break;
            case "melon":
                Melon.acquire(target, maid);
                break;
            default: break;
        }
    }
        ...
        for (let i = 0; i < 3; i++) {
            system.runTimeout(() => {
                try {
                    ...
                    if (target !== undefined) {
                        if (pointInArea_3D(target.location.x, target.location.y, target.location.z, maid.location.x - 2, maid.location.y - 5, maid.location.z - 2, maid.location.x + 2, maid.location.y + 5, maid.location.z + 2)) {
                            switch (work) {
                                case EntityMaid.Work.farm:
                                    Farm.acquire(target, maid);
                                    break;
                                case EntityMaid.Work.melon:
                                    Melon.acquire(target, maid);
                                    break;
                                case EntityMaid.Work.cocoa:
                                    Cocoa.acquire(target, maid);
                                    break;
                                default: break;
                            }
                        }
...
export class Farm {
    ...
    static placeCorp(dimension, location) {
        ...
        // 放置目标
        let target = dimension.spawnEntity("thlmt:farm", new Vector(location.x + 0.5, location.y + 0.2, location.z + 0.5));
            ...
            if (seedInfos !== undefined) {
                for (let seedInfo of seedInfos) {
                    if (seedInfo.land.includes(landName)) {
                        maid.dimension.setBlockPermutation(location, BlockPermutation.resolve(seedInfo.block, seedInfo.state));
                        return true;
                    }
                }
            }
        ...
        if (target.getDynamicProperty("is_seed") === true) {
            ...
            if (type === undefined) {
                ...
                for (let delta of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
                    ...
                    if (block !== undefined) {
                        if (names[block.typeId] === undefined)
                            names[block.typeId] = 1;
                        else
                            names[block.typeId]++;
                    }
                }
    ...
    static search(maid, _range = 6) {
        ...
        function* newSearchJob() {
            ...
            for (let ix = 0; ix < range; ix = ix > 0 ? -ix : 2 - ix) {
                // 获取作物方块
                let lands = dimension.getBlocks(new BlockVolume(new Vector(location.x + ix, location.y - 2, location.z - range), new Vector(location.x + ix + 1, location.y + 2, location.z + range)), {
...
Farm.maxLack = 2; // 进入慢扫描的连续缺目标次数
Farm.lackStep = 5; // 慢扫描等待步数
Farm.minCount = 8; // 缺目标的最大目标数
Farm.maxCount = 25; // 单次扫描获取的最大目标数 到达该数量则停止扫描 确定标准为一个目标实体生命周期内，女仆能收集到的最大目标数
// 甘蔗
class SugarCane {
    /**
     * 放置目标, 位置应为整数方块坐标
    ...
    static place(dimension, location) {
        ...
        // 放置目标
        dimension.spawnEntity("thlmt:sugar_cane", new Vector(location.x + 0.5, location.y + 0.2, location.z + 0.5));
    }
    /**
     * 收集甘蔗
     * @param {Entity} target
...
SugarCane.maxLack = 2; // 进入慢扫描的连续缺目标次数
SugarCane.lackStep = 5; // 慢扫描等待步数
SugarCane.minCount = 10; // 缺目标的最大目标数
SugarCane.maxCount = 40; // 单次扫描获取的最大目标数 到达该数量则停止扫描 确定标准为一个目标实体生命周期内，女仆能收集到的最大目标数
// 西瓜、南瓜
export class Melon {
    /**
     * 放置目标, 位置通常为整数方块坐标
    ...
    static place(dimension, location) {
        ...
        // 放置目标
        dimension.spawnEntity("thlmt:melon", new Vector(location.x + 0.5, location.y + 0.4, location.z + 0.5));
    }
    /**
     * 收集瓜类 进行两次攻击后才会将瓜破坏
     * @param {Entity} target
    ...
    static search(maid, _range = 6) {
        ...
        // 记录点位，搜索时跳过
        for (let target of existedTargets) {
            matrix[Math.floor(target.location.x) - xStart + range][Math.floor(target.location.z) - zStart + range] = true;
        }
        /**
        ...
        for (let ix = 0; ix < range; ix = ix > 0 ? -ix : 1 - ix) {
            system.runTimeout(() => {
                for (let iz = 0; iz < range; iz = iz > 0 ? -iz : 1 - iz) {
                    if (count > this.maxCount)
                        return;
                    if (matrix[ix + range][iz + range] !== undefined)
                        continue;
                    let x = xStart + ix;
                    let z = zStart + iz;
                    let _y = y;
                                        ...
                                        case 2:
                                            {
                                                ...
                                                count++;
                                                if (iz > 0)
                                                    matrix[ix + range][iz + range - 1] = true;
                                                y = _y;
                                            }
                                            ;
                                            break;
                                        ...
                                        case 4:
                                            {
                                                ...
                                                count++;
                                                if (ix > 0)
                                                    matrix[ix + range - 1][iz + range] = true;
                                                y = _y;
                                            }
                                            ;
                                            break;
...
Melon.maxLack = 3; // 进入慢扫描的连续缺目标次数
Melon.lackStep = 5; // 慢扫描等待步数
Melon.minCount = 8; // 缺目标的最大目标数
Melon.maxCount = 20; // 单次扫描获取的最大目标数 到达该数量则停止扫描 确定标准为一个目标实体生命周期内，女仆能收集到的最大目标数
// 可可豆
export class Cocoa {
    /**
     * 放置目标, 位置通常为整数方块坐标
    ...
    static place(dimension, location, direction) {
        ...
        // 放置目标
        dimension.spawnEntity("thlmt:cocoa", new Vector(location.x + 0.5, location.y + 0.5, location.z + 0.5))
...
Cocoa.maxLack = 3; // 进入慢扫描的连续缺目标次数
Cocoa.lackStep = 5; // 慢扫描等待步数
Cocoa.minCount = 8; // 缺目标的最大目标数
Cocoa.maxCount = 20; // 单次扫描获取的最大目标数 到达该数量则停止扫描 确定标准为一个目标实体生命周期内，女仆能收集到的最大目标数
//# sourceMappingURL=MaidTarget.js.map