...
MaidManager.Interact = class {
    ...
    */
    ...
    static onSmartSlabRecycleEvent(event) {
        ...
        if (item === undefined || item.typeId !== "touhou_little_maid:smart_slab_empty")
            return;
        // 将女仆转为lore
        let lore = EntityMaid.toLore(maid);
        // 清除女仆
...
MaidManager.Shedule = class {
    ...
    static returnHomeEvent(event) {
        ...
        if (in_home) {
            // 计算范围
            in_home = Tool.pointInArea_2D(maid.location.x, maid.location.z, home_location[0] - homeRadius, home_location[2] - homeRadius, home_location[0] + homeRadius, home_location[2] + homeRadius);
        }
        // 维度不同或超出范围，回家
        world.getDimension(home_location[3]);
        if (!in_home) {
            maid.teleport(new Vector(home_location[0], home_location[1], home_location[2]), { "dimension": world.getDimension(home_location[3]) });
        }
    }
    /**
     * 定时事件
    ...
    static timerEvent(event) {
        ...
        //// 每次
        // 抱起扫描
        if (maid.getProperty("thlm:is_hug"))
            MaidManager.Hug.maidScan(maid);
    ...
    static danmakuAttack(event) {
        ...
        if (target != undefined) {
            ...
            if (target.typeId === "minecraft:phantom") { // 无效：target.getComponent("minecraft:can_fly") !== undefined
                ...
                let direction = new Vector(target.location.x - location.x, target.location.y - location.y, target.location.z - location.z);
                cherryShoot(maid, location, direction, basicDamage, basicDamage / 3, 1);
                return;
            }
            // 默认攻击方式
            let distance = VectorMC.length(new Vector(maid.location.x - maid.target.location.x, maid.location.y - maid.target.location.y, maid.location.z - maid.target.location.z));
...
MaidManager.Hug = class {
    ...
    static summonInteractEntity(maid, player) {
        let hugMaid = player.dimension.spawnEntity("touhou_little_maid:hug_maid", new Vector(player.location.x, player.location.y, player.location.z));
...
//# sourceMappingURL=MaidManager.js.map