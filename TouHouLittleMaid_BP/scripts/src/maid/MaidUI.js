...
class MaidMenuSimple {
    ...
    main() {
        ...
        const form = new mcui.ActionFormData()
            ...
            // 信息
            .body(` | §6Lv.${EntityMaid.Level.get(this.maid)}§r` +
...
class MaidMenuUI {
    ...
    main() {
        ...
        const form = new mcui.ActionFormData()
            ...
            .body(this.maid.id)
            // 女仆信息
            .button(`${String.fromCodePoint(0xE605)} ${health.currentValue.toFixed(0)}/${health.defaultValue}\n` +
            `${String.fromCodePoint(0xE609)} ${EntityMaid.Kill.get(this.maid)} `)
            // Home 模式
        ...
        for (let i = 0; i < EntityMaid.Work.AMOUNT; i++) {
            if (i === work_type)
                form.button({ rawtext: [{ text: "§a" }, { translate: EntityMaid.Work.getLang(i) }] }, EntityMaid.Work.getIMG(i));
            else
                form.button({ translate: EntityMaid.Work.getLang(i) }, EntityMaid.Work.getIMG(i));
        }
        form.show(this.player).then((response) => {
            if (response.selection !== undefined) {
                switch (response.selection) {
                    ...
                    case 1:
                        ...
                        break; // Home 模式
                    case 2:
                        EntityMaid.Pick.set(this.maid, !pick_mode);
                        system.runTimeout(() => { this.main(); }, 1);
                        break; // 拾物模式
                    case 3:
                        EntityMaid.Ride.switchMode(this.maid);
                        system.runTimeout(() => { this.main(); }, 1);
                        break; // 骑乘模式
                    case 4:
                        EntityMaid.Backpack.setInvisible(this.maid, !backpack_invisible);
                        ...
                        break; // 隐藏背包
                    case 5:
                        EntityMaid.Mute.set(this.maid, !mute_mode);
                        system.runTimeout(() => { this.main(); }, 1);
                        break; // 静音模式
                    case 6:
                        this.skinpackSelection();
...
//# sourceMappingURL=MaidUI.js.map