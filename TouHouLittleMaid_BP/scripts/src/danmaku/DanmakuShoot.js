...
export class DanmakuShoot {
    ...
    calculateVelocity(danmaku) {
        ...
        if (this.enable_shoot_location) {
            ...
            s_location = new Vector(s.x + this.thrower_offset.x, s.y + this.thrower_offset.y, s.z + this.thrower_offset.z);
        ...
        if (this.enable_target_location) {
            ...
            t_location = new Vector(t.x + this.target_offset.x, t.y + this.target_offset.y, t.z + this.target_offset.z);
        }
        //// 计算发射者到目标的向量 ////
        if (VectorMC.equals(s_location, t_location)) {
            ...
            if (this.pre_judge && !this.enable_target_location) { // 预瞄，必须给定目标实体   幻翼：我超有挂
                ...
                if (!this.pre_judge_verticle)
                    targetV.y = 0; // 受重力和落地影响，容易误判，竖直方向默认关闭
            ...
            // 不使用预瞄或预瞄点与发射点重合
            return VectorMC.getVector_speed_direction(this.velocity, new Vector(t_location.x - s_location.x, t_location.y - s_location.y, t_location.z - s_location.z));
        }
    }
    aimedShot() {
        ...
        if (this.ownerID !== undefined) {
            danmaku.setOwnerID(this.ownerID);
        }
...
DanmakuShoot.RANDOM = Math.random();
DanmakuShoot.MAX_YAW = 2 * Math.PI;
DanmakuShoot.MIN_FAN_NUM = 2;
//# sourceMappingURL=DanmakuShoot.js.map