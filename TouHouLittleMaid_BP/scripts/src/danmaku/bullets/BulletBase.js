...
export class BulletBase {
    ...
    constructor(world, thrower) {
        // 位置确定
        this.enableThrowerLocation = false; // 是否指定发射位置
        this.thrower_offset = new Vector(0, 0, 0); // 使用实体作为发射位置时的偏移
        this.damage = 6; // 伤害
    ...
    getShootLocation() {
        ...
        try {
            return new Vector(this.thrower.location.x + this.thrower_offset.x, this.thrower.location.y + this.thrower_offset.y, this.thrower.location.z + this.thrower_offset.z);
        }
        catch {
            return undefined;
        }
     ...
     * 为 动量 应用不准确度
     */
    applyInaccuracy(input, inaccuracy) {
        if (inaccuracy === 0) {
            return input;
        }
     ...
     * @param bulletEntity 弹幕实体
     */
    applyThrowerOwner(bulletEntity) {
