...
export class VectorMC {
    ...
    static rotate_axis(source, axis, theta) {
        ...
        return this.add(this.add(this.multiply(source, cos), this.multiply(this.cross(axis_n, source), sin)), this.multiply(this.multiply(axis_n, this.dot(axis_n, source)), (1 - cos)));
    }
    /**
     * 将向量转为字符
     * @param {Vector} vector
