...
export class BlockTool {
    ...
    static runCommand(b, cmd) {
        const l = b.location;
        b.dimension.runCommand(`execute positioned ${l.x} ${l.y} ${l.z} run ${cmd}`);
    }
}
/**
 * 返回 rawtext
...
export function sendMessage2Entity(entity, str, mode = 0, translate = false) {
    ...
    switch (mode) {
        default:
            entity.runCommand(`tellraw @s {"rawtext":[{"${textObject}": "${str}"}]}`);
            break;
        case 1:
            entity.runCommand(`titleraw @s actionbar {"rawtext":[{"${textObject}":"${str}"}]}`);
            break;
    }
}
//# sourceMappingURL=ScarletToolKit.js.map