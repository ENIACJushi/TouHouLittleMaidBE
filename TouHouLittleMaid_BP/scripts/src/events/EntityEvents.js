            ...
            switch (event.eventId.substring(4, 5)) {
                ...
                case ":":
                    switch (event.eventId.substring(5)) {
                        ...
                        // b: box open
                        case "b":
                            MaidManager.boxOpenEvent(event);
                            break;
                        // n: NPC
                        case "n":
                            MaidManager.NPCInteract(event);
                            break;
                        // h: hug
                        case "h":
                            MaidManager.Hug.seatScan(event);
                            break;
                        default: break;
                    }
                    ;
                ...
                case "m":
                    switch (event.eventId.substring(6, 7)) {
                        case "a":
                            MaidManager.Shedule.danmakuAttack(event);
                            break; // a Danmaku Attack
                        case "d":
                            MaidManager.Core.onDeathEvent(event);
                            break; // d Death
                        case "f":
                            MaidManager.Core.onTamed(event);
                            break; // f on tamed
                        case "h":
                            MaidManager.Shedule.returnHomeEvent(event);
                            break; // h Home
                        case "i":
                            MaidManager.Interact.inventoryModeEvent(event);
                            break; // i Inventory mode
                        case "j":
                            MaidManager.Hug.startEvent(event);
                            break; // j Hug
                        case "k":
                            MaidManager.Hug.stopEvent(event);
                            break; // k Hug stop
                        case "l":
                            MaidManager.setLevelEvent(event);
                            break; // l Level
                        case "m":
                            MaidManager.Interact.onInteractEvent(event);
                            break; // m Master interact
                        case "n":
                            MaidManager.onNPCEvent(event);
                            break; // n NPC
                        case "p":
                            MaidManager.Interact.onPhotoEvent(event);
                            break; // p Photo
                        case "s":
                            MaidManager.Interact.sitModeEvent(event);
                            break; // s Sit mode
                        case "t":
                            MaidManager.Shedule.timerEvent(event);
                            break; // t Timer
                        case "u":
                            GarageKit.scan(event);
                            break; // u statues destroy
                        case "v":
                            MaidManager.Interact.onSitEvent(event);
                            break; // v enter sit
                        case "w":
                            MaidManager.Interact.onStandEvent(event);
                            break; // v enter sit
                        case "0":
                            MaidManager.Core.onSpawnEvent(event);
                            break; // 0 Spawn
                        case "1":
                            MaidManager.Interact.onSmartSlabRecycleEvent(event);
                            break; // 1 Smart slab
                        default: break;
                    }
                    break;
                ...
                case "b":
                    switch (event.eventId.substring(6)) {
                        // g: grave
                        case "g":
                            MaidManager.Core.tombstoneAttackEvent(event);
                            break;
                        // t0: type 0 (default)
                        case "t0":
                            MaidManager.Interact.backpackTypeChangeEvent(event, 0);
                            break;
                        // t1: type 1 (small)
                        case "t1":
                            MaidManager.Interact.backpackTypeChangeEvent(event, 1);
                            break;
                        // t2: type 2 (middle)
                        case "t2":
                            MaidManager.Interact.backpackTypeChangeEvent(event, 2);
                            break;
                        // t3: type 3 (big)
                        case "t3":
                            MaidManager.Interact.backpackTypeChangeEvent(event, 3);
                            break;
                    }
                    break;
                case "w":
                    switch (event.eventId.substring(6, 7)) {
                        case "d":
                            GoldMicrowaver.despawnEvent(event);
                            break; // d Despawn
                        case "f":
                            GoldMicrowaver.finishEvent(event);
                            break; // f finish
                        case "i":
                            GoldMicrowaver.interactEventNoItem(event);
                            break; // i interact(NO Item, Not Sneaking)
                        case "s":
                            GoldMicrowaver.interactEventNoItemSneaking(event);
                            break; // i interact(NO Item, Sneaking)
                        default: break;
                    }
                    break;
                default: break;
