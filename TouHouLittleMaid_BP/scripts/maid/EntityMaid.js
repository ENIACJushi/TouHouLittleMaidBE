...
export class EntityMaid{
    ...
    static toLore(maid){
         ...
         * bi: bag id, 临时背包的生物id
         */ 
        let bag_id = this.getBagID(maid);
        if(bag_id !== undefined){
            let bag = world.getEntity(bag_id);
            if(bag !==undefined){
                let bag_type = bag.getComponent("skin_id").value;
                infos["b"] = bag_type;

                // 在地底创建新背包(隐形)
                let temp_bag = MaidBag.create(undefined, bag_type, bag.dimension, 
                    new Vector(location[0], -63, location[2]));
                MaidBag.setInvisible(temp_bag, true);
                infos["bi"] = temp_bag.id;
                system.runTimeout(()=>{
                    // 将物品移入临时背包
                    MaidBag.copy(bag, temp_bag);
                    MaidBag.clear(bag);
                    // 清除旧背包
                    bag.triggerEvent("despawn");
                },1);
            }
        }
    ...
    static fromLore(lore, dimension, location, set_health=false){
        ...
        var new_bag = MaidBag.create(maid, infos["b"], dimension, location);
        var old_id = infos["bi"];
        system.runTimeout(()=>{
        ...
        let interval_id = system.runInterval(()=>{
            Tool.logger("run");
            let old_bag = world.getEntity(old_id);
            if(old_bag !== undefined){
                Tool.logger("run_over");
                MaidBag.copy(old_bag, new_bag);
                old_bag.triggerEvent("despawn");
     ...
     */
    static getBagEntity(maid){
        let id = this.getBagID(maid);
        if(id !== undefined){
            return world.getEntity(id);
        }
        return undefined;
     ...
     */
    static getBag(maid){
        let bag = this.getBagEntity(maid);
        if(bag !== undefined){
            return MaidBag.getContainer(bag);
        }
        return undefined;
    }
    /**
