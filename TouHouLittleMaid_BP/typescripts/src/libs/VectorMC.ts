...
export class VectorMC {
   ...
   * @param {number} theta A numeric expression that contains an angle measured in radians.
   */
  static rotate_axis(source: Vector, axis: Vector, theta: number) {
   ...
   * @returns 
   */
  static getVector_speed_direction(speed: number, direction: Vector) {
    let k = Math.sqrt((speed * speed) / (direction.x * direction.x + direction.y * direction.y + direction.z * direction.z));
    return new Vector(direction.x * k, direction.y * k, direction.z * k);
  }
  /**
   ...
   * @returns {Array}
   */
  static getEulerAngleXZ(v: Vector): Array<any> {
    let z = Math.atan2(v.x, v.y);
   ...
   * @returns {Array} 
   */
  static getEulerAngle(v: Vector): Array<any> {
    let z = Math.atan2(v.y, v.x);
    let y = Math.atan2(v.z, Math.sqrt(v.x * v.x + v.y * v.y));
    if (v.x < 0) {
      // Z 在 +-PI/2 之间
   ...
   * @returns {Array} 
   */
  static getDanmakuAngle(v: Vector): Array<any> {
    let z = Math.atan2(v.y, v.x);
    let y = Math.atan2(v.z, v.x);
    return [z, y];
  }
   ...
   * 获取任意一条与给定非零向量垂直的向量
   */
  static getAnyVerticalVector(vector: Vector): Vector {
    if (vector.x === 0 && vector.y === 0) {
      // 向量为0，返回z轴
      if (vector.z === 0) {
        return new Vector(0, 0, 1);
   ...
   * @returns {Direction}
   */
  static getDirectionByView2D(view: Vector): Direction {
    let temp = (Math.abs(view.x) - Math.abs(view.y) > 0);// |x| > |y|
    if (view.x > 0) {
      if (view.z > 0) {
        return temp ? Direction.East : Direction.South;
   ...
   * @returns {Vector} A 发射的直线弹射物的速度
   */
  static preJudge(A: Vector, B: Vector, Va_len: number, Vb: Vector): Vector {
    let BA = this.sub(A, B);
