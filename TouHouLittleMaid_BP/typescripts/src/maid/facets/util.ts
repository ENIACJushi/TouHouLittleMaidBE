...
/**
 * 杂项工具（对齐原 EntityMaid 顶层静态辅助）
 */
export const Util = {
  /**
   * 格式化输出 rawtext
   */
  formatOutput(maid: Entity): object[] {
    let rawtext: object[] = [];
    // 标题
   ...
   * 判断一个方块是否安全（用于放置女仆）
   */
  isSafeBlock(block: Block | undefined): boolean {
   ...
   * 生成一个随机女仆（当前仍为默认女仆）
   */
  spawnRandomMaid(dimension: Dimension, location: Vector3): Entity {
